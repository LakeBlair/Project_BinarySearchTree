...
public:
  ...
  Value_type& operator[](const Key_type& k) {
      ...
      if (key != end()) {
          ...
          std::pair<Iterator, bool> pair1 = insert(new_pair);
          Iterator i = pair1.first;
          Value_type& p = (*i).second;
          return p;
      }
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type& val) {
      ...
      Iterator key_in_map = find(key);

      if (key_in_map != end()) {
          std::pair<Iterator, bool> p = { key_in_map, false };
          return p;
      }
      else {
          b.insert(val);
          Iterator i = b.find(val);
          std::pair<Iterator, bool> p = { i, true };
          return p;
      }
  }
