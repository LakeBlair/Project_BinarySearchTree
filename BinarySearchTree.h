...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
      if (!node) {
          ...
          Node* nod = new Node(node->datum, nullptr, nullptr);
          nod->left = copy_nodes_impl(node->left);
          nod->right = copy_nodes_impl(node->right);
          return nod;
      }
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
      if (!node) {
          Node* nod = new Node(item, nullptr, nullptr);
          return nod;
      }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      ...
      if (!less(val, node->datum)) {
          ...
          if (!node->left || !less(val, max_element_impl(node->left)->datum)) {
              return node;
          }
          else {
